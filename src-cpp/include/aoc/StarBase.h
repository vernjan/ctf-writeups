...
public:
    ...
    void run_test(size_t expected_result, const string &input_file_name = "input-test.txt") {
        run_star(input_file_name, expected_result);
    }

    void run(size_t expected_result, const string &input_file_name = "input.txt") {

        RunResult result = run_star(input_file_name, expected_result);
        std::cout << "Star: " << star << ": " << result.result << " (" << result.duration << " ms)" << "\n";
    }

    [[nodiscard]] virtual size_t execute(const vector<string> &data) const = 0;
