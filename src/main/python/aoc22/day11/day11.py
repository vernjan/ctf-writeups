...
def monkeys_example():
    return [
        Monkey(0, [79, 98], lambda w: w * 19, lambda w: test_div(w, 23, 2, 3)),
        Monkey(1, [54, 65, 75, 74], lambda w: w + 6, lambda w: test_div(w, 19, 2, 0)),
        Monkey(2, [79, 60, 97], lambda w: w * w, lambda w: test_div(w, 13, 1, 3)),
        Monkey(3, [74], lambda w: w + 3, lambda w: test_div(w, 17, 0, 1)),
    ]


def monkeys():
    return [
        Monkey(0, [72, 64, 51, 57, 93, 97, 68], lambda w: w * 19, lambda w: test_div(w, 17, 4, 7)),
        Monkey(1, [62], lambda w: w * 11, lambda w: test_div(w, 3, 3, 2)),
        Monkey(2, [57, 94, 69, 79, 72], lambda w: w + 6, lambda w: test_div(w, 19, 0, 4)),
        Monkey(3, [80, 64, 92, 93, 64, 56], lambda w: w + 5, lambda w: test_div(w, 7, 2, 0)),
        Monkey(4, [70, 88, 95, 99, 78, 72, 65, 94], lambda w: w + 7, lambda w: test_div(w, 2, 7, 5)),
        Monkey(5, [57, 95, 81, 61], lambda w: w * w, lambda w: test_div(w, 5, 1, 6)),
        Monkey(6, [79, 99], lambda w: w + 2, lambda w: test_div(w, 11, 3, 1)),
        Monkey(7, [68, 98, 62], lambda w: w + 3, lambda w: test_div(w, 13, 5, 6)),
    ]


def test_div(worry, divisor, success, failure):
