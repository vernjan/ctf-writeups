...
def star1(lines: list[str]):
    ...
    cubes_position_map = _get_cubes_position_map(cubes)
...
def star2(lines: list[str]):
    ...
    for bottom_cube in cubes:
        ...
        for p in bottom_cube.positions:
            del cpm[p]
        while queue:
            cube = queue.pop(0)
...
def _get_cubes_position_map(cubes):
    return {p: c for c in cubes for p in c.positions}
